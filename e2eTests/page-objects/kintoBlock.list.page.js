...
class KintoBlockList extends Page {
  ...
  getKbCardImg(id) {
    return $(`.kintoblock-list [data-test='kb-card-id-${id}'] .top .left>img`)
  }
  ...
  getBranchFromBranchList(id, index) {
    return $(
      `[data-test='kb-card-id-${id}'] [data-test='branch-list']>button:nth-child(${index}) .tag-item-text`
    )
  }

  getTagFromTagsList(id, index) {
    return $(
      `[data-test='kb-card-id-${id}'] [data-test='tag-list']>button:nth-child(${index}) .tag-item-text`
    )
  }

  getTagVersionFromTagList(id, index) {
    return $(
      `[data-test='kb-card-id-${id}'] [data-test='tag-list']>button:nth-child(${index}) .tag-and-commit>div:nth-child(1)`
    )
  }

  getTagTimeAndDateFromTagList(id, index) {
    return $(
      `[data-test='kb-card-id-${id}'] [data-test='tag-list']>button:nth-child(${index}) .date`
    )
  }
